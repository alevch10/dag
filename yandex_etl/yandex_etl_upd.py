    ...
    **context
    ...
    try:
        ...
        if 'failed_file' in resp_json and resp_json['failed_file']:
            logging.warning(f"Failed file: {resp_json['failed_file']}, line: {resp_json.get('failed_line')}")
        
        if 'last_successful_file' in resp_json and resp_json['last_successful_file']:
            logging.info(f"Last successful file: {resp_json['last_successful_file']}, line: {resp_json.get('last_successful_line')}")
        
        if 'error_details' in resp_json and resp_json['error_details']:
            logging.error(f"Error details: {resp_json['error_details']}")
        
        # ---------- Сохраняем результат в XCom для передачи в другие задачи ----------
        ti = context['ti']
        ...
        if resp_json.get('status') == 'success':
            ...
            # Если статус не success, но код HTTP 2xx, считаем это ошибкой бизнес-логики
            error_msg = resp_json.get('message', 'Неизвестная ошибка')
            raise ValueError(f"Не успешный статус ответа: {error_msg}")
